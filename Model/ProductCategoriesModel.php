...
class ProductCategoriesModel
{
    ...
    {
        $stmt = $this->conn->prepare("SELECT p.ProductName, p.Description, p.Price, p.Quantity, p.SoldQuantity FROM Products p INNER JOIN Product_Categories pc ON p.ProductID = pc.ProductID WHERE pc.CategoryID = ?");
